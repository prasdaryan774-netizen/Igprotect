...
function telegramRequest($url, $data, $multipart = false) {
    $ch = curl_init($url);
    ...
    if ($http !== 200 || !isset($result['ok']) || !$result['ok']) {
        error_log("Telegram API error: HTTP {$http} | {$response}");
        return false;
    }
...
function sendTelegramPhoto($path, $caption = '') {
    ...
    error_log("PHOTO FILE: " . $path);
    error_log("PHOTO SIZE: " . filesize($path));
    ...
    error_log("PHOTO HTTP: $http");
    error_log("PHOTO CURL ERROR: $error");
    error_log("PHOTO RESPONSE: $response");
...
function sendTelegramAudio($path, $caption = '', $mime = 'audio/webm', $ext = 'webm') {
    if (!is_file($path)) {
        error_log("Telegram audio file not found: " . $path);
        return false;
    }
    ...
    $data = [
        'chat_id' => TELEGRAM_CHAT_ID,
        'audio' => new CURLFile($path, $mime, $fn),
        'caption' => $caption,
        'parse_mode' => 'HTML'
    ];

    $result = telegramRequest($url, $data, true);
    ...
    if (!$result) {
        ...
        $data2 = [
            'chat_id' => TELEGRAM_CHAT_ID,
            'document' => new CURLFile($path, $mime, $fn),
            'caption' => $caption . ' [doc]',
            'parse_mode' => 'HTML'
        ];

        return telegramRequest($url2, $data2, true);
    }
